...
export function Board({ members }: Props) {
  ...
  if (error) return (
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', maxWidth: 360 }}>
        <div style={{ width: 62, height: 62, borderRadius: 16, background: 'rgba(229,83,75,.1)', border: '1px solid rgba(229,83,75,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>
        ...
        <button onClick={refetch} style={{ display: 'flex', alignItems: 'center', gap: 7, height: 36, padding: '0 15px', marginTop: 20, borderRadius: 8, background: 'var(--surface-3)', border: '1px solid var(--border-2)', color: 'var(--text)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          Try again
        </button>
      </div>
    </div>
  ...
  return (
    <>
      {/* Topbar */}
      <header style={{ height: 56, flexShrink: 0, borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 14, padding: '0 18px', background: 'var(--surface)' }}>
        ...
        <div style={{ position: 'relative', marginLeft: 'auto', width: 280, maxWidth: '34vw' }}>
          ...
          <input
            ...
            style={{ width: '100%', height: 34, padding: '0 12px 0 32px', borderRadius: 8, background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 13, outline: 'none' }}
          />
        </div>

        {/* New task */}
        <button onClick={() => setModalOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, height: 34, padding: '0 13px 0 11px', borderRadius: 8, background: 'var(--accent)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
      ...
      {/* Toolbar / stats */}
      <div style={{ height: 54, flexShrink: 0, borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10, padding: '0 18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginLeft: 6 }}>
          ...
          {overdue > 0 && (
            <span style={{ fontSize: 12, color: 'var(--red)', background: 'rgba(229,83,75,.1)', border: '1px solid rgba(229,83,75,.25)', borderRadius: 6, padding: '3px 9px', fontWeight: 500 }}>
              {overdue} overdue
            </span>
          )}
        </div>
      ...
      {tasks.length === 0 && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', maxWidth: 340 }}>
            <div style={{ width: 62, height: 62, borderRadius: 16, background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>
            ...
            <button onClick={() => setModalOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, height: 36, padding: '0 15px', borderRadius: 8, background: 'var(--accent)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
      ...
      {tasks.length > 0 && (
        <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          ...
          <DragOverlay>
            {activeTask && (
              <TaskCard task={activeTask} members={members} onClick={() => {}} isOverlay />
            )}
          </DragOverlay>
        </DndContext>
      )}
      ...
      {selectedTask && (
        <TaskDetailPanel
          ...
          onUpdate={async (changes, old) => {
            const updated = await updateTask(selectedTask.id, changes, old)
...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span style={{ fontSize: 12, color: 'var(--text-2)', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 6, padding: '3px 9px', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
      {children}
    </span>
  )
}